...
#define HID_USAGE_SENSOR_QUATERNION				0x200483
                              ...
                              long mask)
{
        ...
        *val2 = 0;
        ...
        case IIO_CHAN_INFO_SCALE:
                *val = sensor_state->channel_attributes[CHANNEL_QUATERNION].units;
                ret_type = IIO_VAL_INT;
                break;
        case IIO_CHAN_INFO_OFFSET:
                *val = hid_sensor_convert_exponent(
                        sensor_state->channel_attributes[CHANNEL_QUATERNION].unit_expo);
                ret_type = IIO_VAL_INT;
                break;
                                ...
                                void *priv)
{
        ...
        switch (usage_id) {
        case HID_USAGE_SENSOR_QUATERNION: /* add other sensors */
                offset = usage_id - HID_USAGE_SENSOR_QUATERNION;
                sensor_state->channel_vals[CHANNEL_QUATERNION + offset] =
                                                *(u32 *)raw_data;
                ret = 0;
        break;
        default:
                                ...
                                struct sensor_state *st)
{
        ...
        for (i = 0; i < CHANNEL_MAX; ++i) {
                ret = sensor_hub_input_get_attribute_info(hsdev,
                                HID_INPUT_REPORT,
                                usage_id,
                                HID_USAGE_SENSOR_QUATERNION + i,
                                &st->channel_attributes[CHANNEL_QUATERNION + i]);
                if (ret < 0)
                        break;
                sensor_adjust_channel_bit_mask(channels,
                                CHANNEL_QUATERNION + i,
                                st->channel_attributes[CHANNEL_QUATERNION + i].size);
        }
...
{
        int ret = 0;
        static const char *name = "orientation";
        ...
        ret = sensor_parse_report(pdev, hsdev, channels,
                                        HID_USAGE_SENSOR_DEVICE_ORIENTATION, sensor_state);
        if (ret) {
        ...
        ret = sensor_hub_register_callback(hsdev, HID_USAGE_SENSOR_DEVICE_ORIENTATION,
...
MODULE_DESCRIPTION("HID Sensor Device Orientation");
